...
((SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io'),2), -- 李燕容`新增為教練，並且年資設定為2年
((SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io'),2), -- 肌肉棒子`新增為教練，並且年資設定為2年
((SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io'),2); -- Q太郎`新增為教練，並且年資設定為2年
...
WHERE user_id = (SELECT id FROM "USER" WHERE name = '肌肉棒子'); -- OR (SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io')

    -- 2. 教練`Q太郎` 的經驗年數為5年

UPDATE "COACH" 
SET experience_years = '5'
WHERE user_id = (SELECT id FROM "USER" WHERE name = 'Q太郎'); -- OR (SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io')
    ...
    (SUM(cp.purchased_credits) - 
        (SELECT COUNT(*) 
         ...
         WHERE user_id = cp.user_id 
           AND STATUS = '上課中')) AS remaining_credit
FROM 
    "CREDIT_PURCHASE" cp
LEFT JOIN 
    "COURSE_BOOKING" cb
...
WHERE 
    cp.user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') AND cb.status = '上課中'
GROUP BY 
    cp.user_id;
...
JOIN "COACH" c on c.id = cls.coach_id
...
JOIN "CREDIT_PACKAGE" cp2 
    ON cp2.id = cp.credit_package_id
WHERE cp.created_at BETWEEN '2024-11-01 00:00:00' AND '2024-11-30 23:59:59'
GROUP BY cp2.name;
